...
- **A download client and its *arr app must resolve the same container path to the same host directory.** The *arr asks the client where a file is and gets back a path in the *client's* namespace, then resolves it against its own mounts — if the two disagree, the *arr lands in the wrong real directory. If that directory exists and is readable, this presents as a plain "missing file" import failure, not an error. Compare `docker inspect` mounts across both containers for the identical path string before theorizing.
